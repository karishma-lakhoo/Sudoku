...
int convertB25CharacterToInt(char element){
    int integerEquivalent;
    if(element == 'A'){
        integerEquivalent = 10;
    }
    if(element == 'B'){
        integerEquivalent = 11;
    }
    if(element == 'C'){
        integerEquivalent = 12;
    }
    if(element == 'D'){
        integerEquivalent = 13;
    }
    if(element == 'E'){
        integerEquivalent = 14;
    }
    if(element == 'F'){
        integerEquivalent = 15;
    }
    if(element == 'G'){
        integerEquivalent = 16;
    }
    if(element == 'H'){
        integerEquivalent = 17;
    }
    if(element == 'I'){
        integerEquivalent = 18;
    }
    if(element == 'J'){
        integerEquivalent = 19;
    }
    if(element == 'K'){
        integerEquivalent = 20;
    }
    if(element == 'L'){
        integerEquivalent = 21;
    }
    if(element == 'M'){
        integerEquivalent = 22;
    }
    if(element == 'N'){
        integerEquivalent = 23;
    }
    if(element == 'O'){
        integerEquivalent = 23;
    }
...
char convertIntToB25Character(int element){
    char Base25Equivalent;
    if(element == 10){
        Base25Equivalent = 'A';
    }
    if(element == 11){
    ...
    if(element == 24){
        ...
        Base25Equivalent = static_cast<char>(element);
    }
    return Base25Equivalent;
}
...
vector<vector<int>> B25ArrayToIntVector(char inputArray[ROWS][COLUMNS]){
    vector<vector<int>> inputVector(N, vector<int> (N, 0));
    for(int i = 0; i < ROWS; ++i) {
        for(int j = 0; j < COLUMNS; ++j) {
            inputVector[i][j] = convertB25CharacterToInt(inputArray[i][j]);
        }
    }
    return inputVector;
}

vector<vector<char>> IntToB25Vector(vector<vector<int>> &inputVector){
    vector<vector<char>> inputCharVector(N, vector<char> (N, 0));
    for(int i = 0; i < ROWS; ++i) {
        for(int j = 0; j < COLUMNS; ++j) {
            inputCharVector[i][j] = convertB25CharacterToInt(inputVector[i][j]);
        }
    }
...
//    printArray(inputArray);
    ...
    intInputVector = B25ArrayToIntVector(inputArray);
//    printVector(intInputVector);
    vector<vector<char>> charOutputVect;
    if(solve(intInputVector)) {
//        cout << "yay!" << endl;
        charOutputVect = IntToB25Vector(intInputVector);
